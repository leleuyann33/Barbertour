...
async function refreshMarkers(events) {
    if (!events || !events.length) {
        console.warn("Pas d'�v�nements � afficher.");
        return;
    }
    console.log("Rendu des marqueurs pour", events.length, "�v�nements.");
    ...
    events.forEach(event => {
        ...
        let cleanTitle = title.replace(/Option/gi, "").replace(/BSQ/gi, "").replace(/"GB"/gi, "").trim().toUpperCase();
