...
document.querySelectorAll('.date-item, #theater-frame, .poster-box, .months-grid').forEach(el => {
...
async function initApp() {
    ...
    if (mainVideo) {
        mainVideo.volume = 0.15; // Force 15% volume
        
        // Auto-pause when scrolled out of view (using a more robust check)
        window.addEventListener('scroll', () => {
            const rect = mainVideo.getBoundingClientRect();
            const isVisible = (rect.top < window.innerHeight && rect.bottom >= 0);
            if (!isVisible && !mainVideo.paused) {
                mainVideo.pause();
                console.log("Vidéo mise en pause (hors écran)");
            }
        });
    }
}
